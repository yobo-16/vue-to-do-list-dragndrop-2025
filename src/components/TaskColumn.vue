...
<template>
  <div
    class="task-column bg-gray-100 p-4 rounded-lg shadow-md"
    :class="{ 'bg-indigo-100 border-indigo-400 border-2': isOver }"
  ...
  >
    ...
    <div ref="container" class="min-h-[80px] flex flex-col gap-2">
      ...
      >
        ...
        <div class="my-4">
          <p class="text-sm text-gray-400">Move to:</p>
          <select
            v-model="project.status"
            @change="emit('move-between-columns', { project, newStatus: project.status })"
            class="px-2 py-1 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
